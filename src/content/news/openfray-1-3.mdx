...
When a player names a spell, look it up from the console’s header. OpenFray 1.3
adds reference search alongside stable labels for repeated creatures and recovery
controls for interrupted fights.
...
Search also finds creatures, saved characters, and console navigation. Results
include creatures from every enabled library and the available spells.
...
Remove Goblin B from the board and Goblin C keeps its letter. You don’t have to
rename the remaining creatures when one leaves the fight.
...
Encounter saving is already included with a free account. Prepare the party and
creatures in advance, or save a fight in progress to restore next session on
whichever device you bring.
